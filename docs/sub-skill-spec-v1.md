...
> **状态**:提议(draft) · **作者**:行业分析模块 · **日期**:2026-04-30
...
| 字段 | 类型 | 必/选 | 说明 |
|---|---|---|---|
| `request_id` | string | 必 | 一次完整 forecast 的追踪 id |
| `schema_version` | string | 必 | 输出 schema 版本(如 `module_output_v1`) |
| `ticker` | string | 必 | 股票代码,A 股为 6 位数字 |
| `stock_name` | string | 选 | 股票名称(子 skill 应能从 ticker 自查) |
| `market` | string | 选 | 市场,默认 `A股`,枚举:`A股 / 港股 / 美股` |
| `analysis_date` | string | 必 | **数据截止日**,YYYY-MM-DD,严禁使用之后数据 |
| `forecast_horizon` | string | 选 | 预测窗口,枚举见下,默认 `20d` |
| `current_price` | number | 选 | 该日收盘价 |
